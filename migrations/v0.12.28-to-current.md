...
**Dynamic form** — evaluates a lambda at runtime to select which screen to show.
Use this when the correct start screen depends on module state:
```kotlin
graph("content") {
    entry(
        route = { store ->
...
The lambda receives a `StoreAccessor` so it can read any module state. It returns
any `NavigationNode` — a `Screen`, `Modal`, or another `Graph` reference.
...
entry(
    ...
    route = { store ->
        // slow async check — loading modal shown after 500ms
        val ready = store.selectState<StartupState>().value.isReady
        if (ready) DashboardScreen else OnboardingScreen
    }
)
```
...
**Notes:** Both forms register the screen(s) as navigatable destinations automatically,
so there is no need to also add them to `screens()`. However, any additional screens
in the graph that are not the entry screen must still be registered via `screens()`.
