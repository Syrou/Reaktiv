...
/**
 * Common URL encoder implementation that works across all KMP targets
 * Uses manual encoding to ensure consistency across platforms
 */
class CommonUrlEncoder : UrlEncoder {
    
    // Characters that are safe in URL paths (more restrictive)
    private val pathSafeChars = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789-._~".toSet()
    
    // Characters that are safe in URL query parameters (less restrictive)
    ...
    override fun decode(encoded: String): String {
        ...
        while (i < encoded.length) {
            when {
                ...
                encoded[i] == '+' -> {
                    // Handle space encoding in query parameters
                    result.append(' ')
                    i++
                }
                else -> {
    ...
    private fun encodeString(value: String, safeChars: Set<Char>): String {
        ...
        for (char in value) {
            when {
                char in safeChars -> result.append(char)
                char == ' ' -> result.append("%20") // Always use %20 for spaces (more reliable than +)
                else -> {
