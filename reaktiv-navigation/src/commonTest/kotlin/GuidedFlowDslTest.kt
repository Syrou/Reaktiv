...
class GuidedFlowDslTest {
    ...
    @Test
    fun `should handle step modification and navigation in sequence`() =
        runTest(timeout = 5.toDuration(DurationUnit.SECONDS)) {
            val testDispatcher = StandardTestDispatcher(testScheduler)
            val store = createStore {
                module(createTestNavigationModule())
            ...
            val modifiedDefinition = stateAfterOperations.guidedFlowModifications[Route.TestFlow]
            assertNotNull(modifiedDefinition)
            assertEquals(4, modifiedDefinition.steps.size)
        }

    @Test
    fun `should support replace step and navigate operations`() =
        runTest(timeout = 5.toDuration(DurationUnit.SECONDS)) {
            val testDispatcher = StandardTestDispatcher(testScheduler)
            val store = createStore {
                module(createTestNavigationModule())
    ...
    fun `should remove step by type`() = runTest(timeout = 5.toDuration(DurationUnit.SECONDS)) {
        ...
        val modifiedDefinition = stateAfter.guidedFlowModifications[Route.TestFlow]
        assertNotNull(modifiedDefinition)
        assertEquals(2, modifiedDefinition.steps.size)

        // Verify DslTestScreen2 is no longer in the flow
        val remainingSteps = modifiedDefinition.steps.map {
            when (it) {
    ...
    fun `should replace step by type`() = runTest(timeout = 5.toDuration(DurationUnit.SECONDS)) {
        ...
        val modifiedDefinition = stateAfter.guidedFlowModifications[Route.TestFlow]
        assertNotNull(modifiedDefinition)
        assertEquals(3, modifiedDefinition.steps.size)

        // Verify the replacement at index 1 (original DslTestScreen2 position)
        val replacedStep = modifiedDefinition.steps[1]
    ...
    @Test
    fun `should update step params by type with raw parameters`() =
        runTest(timeout = 5.toDuration(DurationUnit.SECONDS)) {
            val testDispatcher = StandardTestDispatcher(testScheduler)
            val store = createStore {
                module(createTestNavigationModule())
            ...
            val updatedStep = modifiedDefinition.steps[1] // DslTestScreen2 is at index 1
    ...
    @Test
    fun `should update step params by type with typed parameters`() =
        runTest(timeout = 5.toDuration(DurationUnit.SECONDS)) {
            val testDispatcher = StandardTestDispatcher(testScheduler)
            val store = createStore {
                module(createTestNavigationModule())
            ...
            val updatedStep = modifiedDefinition.steps[2] // DslTestScreen3 is at index 2
            val params = updatedStep.getParams()
    ...
    @Test
    fun `should handle type-based operations when step not found using IfExists methods`() =
        runTest(timeout = 5.toDuration(DurationUnit.SECONDS)) {
            val testDispatcher = StandardTestDispatcher(testScheduler)
            val store = createStore {
                module(createTestNavigationModule())
    ...
    @Test
    fun `should throw exceptions when type-based operations cannot find step type`() =
        runTest(timeout = 5.toDuration(DurationUnit.SECONDS)) {
            val testDispatcher = StandardTestDispatcher(testScheduler)
            val store = createStore {
                module(createTestNavigationModule())
    ...
    @Test
    fun `should verify successful type-based operations return true with IfExists methods`() =
        runTest(timeout = 5.toDuration(DurationUnit.SECONDS)) {
            val testDispatcher = StandardTestDispatcher(testScheduler)
            val store = createStore {
                module(createTestNavigationModule())
            ...
            val updatedStep =
                modifiedDefinition.steps[0] // DslTestScreen2 is now at index 0 after DslTestScreen1 was removed
            assertEquals("value", updatedStep.getParams().getString("test"))

            // DslTestScreen3 should have been replaced with DslTestScreen1
            val replacedStep = modifiedDefinition.steps[1] // The replaced step
            assertTrue(replacedStep is GuidedFlowStep.TypedScreen)
            assertEquals(DslTestScreen1::class.qualifiedName, replacedStep.screenClass)
    ...
    @Test
    fun `should verify full round-trip of typed parameters with serialization and deserialization`() =
        runTest(timeout = 5.toDuration(DurationUnit.SECONDS)) {
            val testDispatcher = StandardTestDispatcher(testScheduler)
            val store = createStore {
                module(createTestNavigationModule())
            ...
            assertEquals(
                Params.of("source" to "test", "timestamp" to 1234567890),
                params.getTyped<Params>("rawMetadata")
            )
    ...
    @Test
    fun `should fail gracefully when trying to serialize non-serializable types`() =
        runTest(timeout = 5.toDuration(DurationUnit.SECONDS)) {
            val testDispatcher = StandardTestDispatcher(testScheduler)
            val store = createStore {
                module(createTestNavigationModule())
            ...
            try {
                ...
                assertTrue(
                    e.message?.contains("Serializer") == true || e.message?.contains("serializable") == true,
    ...
    @Test
    fun `should verify typed parameters work in actual navigation scenario`() =
        runTest(timeout = 5.toDuration(DurationUnit.SECONDS)) {
    ...
    @Test
    fun `should reset guided flow modifications after completion`() =
        runTest(timeout = 10.toDuration(DurationUnit.SECONDS)) {
            ...
            val modifiedDefinition = stateAfterModification.guidedFlowModifications[Route.TestFlow]
            ...
            assertNull(
                stateAfterCompletion.activeGuidedFlowState,
                "Active guided flow should be cleared after completion"
            )

            // Verify modifications are cleared (no runtime definition exists)
            assertNull(
                stateAfterCompletion.activeGuidedFlowState,
                "Flow state should be completely cleared including runtime modifications"
            )
            ...
            // Second run should start with clean state (no runtime modifications)
            val freshModifications = stateSecondRun.guidedFlowModifications[Route.TestFlow]
            assertNull(freshModifications, "Second run should start with clean state (no runtime modifications)")
        }
}