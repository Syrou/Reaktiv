...
class GuidedFlowTest {
    ...
    @Test
    fun `should start guided flow and navigate to first step`() =
    ...
    @Test
    fun `should exit guided flow when calling previous step from first step`() =
        runTest(timeout = 5.toDuration(DurationUnit.SECONDS)) {
            val testDispatcher = StandardTestDispatcher(testScheduler)
            val store = createStore {
                module(createTestNavigationModule())
    ...
    @Test
    fun `should handle route-based guided flow with query parameters`() =
        runTest(timeout = 5.toDuration(DurationUnit.SECONDS)) {
            val testDispatcher = StandardTestDispatcher(testScheduler)
            val store = createStore {
                module(createTestNavigationModule())
    ...
    @Test
    fun `should access state in onComplete for conditional navigation`() =
        runTest(timeout = 5.toDuration(DurationUnit.SECONDS)) {
            val testDispatcher = StandardTestDispatcher(testScheduler)
            ...
            assertEquals(
                TestWelcomeScreen,
                state.currentEntry.screen,
                "Tutorial completed user should navigate to welcome"
            )
            ...
            assertEquals(
                ...
                state.currentEntry.screen,
                "Premium VIP user should navigate to preferences"
            )
    ...
    @Test
    fun `test guided flow parameters with content URI preserves encoding`() =
        runTest(timeout = 5.toDuration(DurationUnit.SECONDS)) {
            val testDispatcher = StandardTestDispatcher(testScheduler)
            val contentUri =
                "content://com.google.android.apps.docs.storage/document/acc%3D9%3Bdoc%3Dencoded%3Dq3kaiKNyhntVpRozJ-eI-R1HG6TXaN_W7vafc216hOscCol3D2WPTL4kvwY%3D"
    ...
    @Test
    fun `should use modified step parameters and onComplete handler`() =
        runTest(timeout = 10.toDuration(DurationUnit.SECONDS)) {
            ...
            assertEquals(
                TestPreferencesScreen,
                finalState.currentEntry.navigatable,
                "Should navigate to TestPreferencesScreen from modified onComplete"
            )
            assertEquals(null, finalState.activeGuidedFlowState, "Guided flow should be cleared after completion")
        }

    @Test
    fun `should reset to original flow definition after completion and work correctly on subsequent runs`() =
        runTest(timeout = 15.toDuration(DurationUnit.SECONDS)) {
            ...
            assertEquals(
                uri,
                stateAfterFirstStart.currentEntry.params.getString("DEEPLINK"),
                "First run should have modified DEEPLINK"
            )
            assertEquals(
                "FIRST",
                stateAfterFirstStart.currentEntry.params.getString("RUN"),
                "First run should have RUN=FIRST"
            )
            ...
            assertEquals(
                TestPreferencesScreen,
                stateAfterFirstCompletion.currentEntry.navigatable,
                "First run should navigate to TestPreferencesScreen"
            )
            assertEquals(
                null,
                stateAfterFirstCompletion.activeGuidedFlowState,
                "Flow should be cleared after first completion"
            )
            ...
            assertEquals(
                null,
                stateAfterSecondStart.currentEntry.params.getString("DEEPLINK"),
                "Second run should NOT have DEEPLINK parameter"
            )
            assertEquals(
                null,
                stateAfterSecondStart.currentEntry.params.getString("RUN"),
                "Second run should NOT have RUN parameter"
            )
            ...
            assertEquals(
                null,
                stateAfterSecondCompletion.activeGuidedFlowState,
                "Flow should be cleared after second completion"
            )

            // The second run should navigate to TestHomeScreen (original onComplete behavior)
            assertEquals(
                TestHomeScreen,
                stateAfterSecondCompletion.currentEntry.navigatable,
                "Second run should navigate to TestHomeScreen (original behavior)"
            )
            ...
            assertEquals(
                "https://different.com",
                stateAfterThirdStart.currentEntry.params.getString("DEEPLINK"),
                "Third run should have new DEEPLINK"
            )
            assertEquals(
                "THIRD",
                stateAfterThirdStart.currentEntry.params.getString("RUN"),
                "Third run should have RUN=THIRD"
            )
            ...
            assertEquals(
                TestProfileScreen,
                finalState.currentEntry.navigatable,
                "Third run should navigate to TestProfileScreen"
            )
            assertEquals(null, finalState.activeGuidedFlowState, "Flow should be cleared after third completion")
        }

    @Test
    fun `should apply modifications and navigate in same DSL call - reproduce user bug`() =
        runTest(timeout = 10.toDuration(DurationUnit.SECONDS)) {
            val testDispatcher = StandardTestDispatcher(testScheduler)
            var modifiedOnCompleteTriggered = false
            ...
            assertEquals(
                TestProfileScreen,
                state.currentEntry.navigatable,
                "Should navigate to TestProfileScreen (step 1)"
            )
            assertEquals(1, state.activeGuidedFlowState?.currentStepIndex)
            ...
            assertEquals(
                uri,
                deeplinkParam,
                "TestProfileScreen should receive the modified DEEPLINK parameter from the guided flow DSL"
            )
        }


    @Test
    fun `should apply modified step parameters to all steps - reproduce user exact scenario`() =
        runTest(timeout = 15.toDuration(DurationUnit.SECONDS)) {
            val testDispatcher = StandardTestDispatcher(testScheduler)
    ...
    @Test
    fun `should handle multiple flows with different modifications simultaneously`() =
        runTest(timeout = 15.toDuration(DurationUnit.SECONDS)) {
            val testDispatcher = StandardTestDispatcher(testScheduler)
            ...
            assertTrue(
                stateAfterModifications.guidedFlowModifications.containsKey("test-flow"),
                "Should have modifications for test-flow"
            )
            assertTrue(
                stateAfterModifications.guidedFlowModifications.containsKey("builder-flow"),
                "Should have modifications for builder-flow"
            )
            ...
            assertEquals(
                TestProfileScreen,
                state.currentEntry.navigatable,
                "Should navigate to TestProfileScreen (signup completion)"
            )
            ...
            assertEquals(
                TestWelcomeScreen,
                state.currentEntry.navigatable,
                "Should start on TestWelcomeScreen for onboarding"
            )

            // Verify onboarding-specific parameters (different from signup!)
            assertEquals(
                onboardingDeeplink,
                state.currentEntry.params.getString("DEEPLINK"),
                "Should have onboarding deeplink"
            )
            assertEquals(
                "ONBOARDING",
                state.currentEntry.params.getString("FLOW_TYPE"),
                "Should have onboarding flow type"
            )
            assertEquals(
                "onboarding-456",
                state.currentEntry.params.getString("USER_ID"),
                "Should have onboarding user ID"
            )
            ...
            assertEquals(
                TestPreferencesScreen,
                state.currentEntry.navigatable,
                "Should navigate to TestPreferencesScreen (onboarding completion)"
            )
            ...
            val finalState = store.selectState<NavigationState>().first()
            assertTrue(
                finalState.guidedFlowModifications.containsKey("test-flow"),
                "test-flow modifications should be preserved with CLEAR_NONE"
            )
            assertTrue(
                finalState.guidedFlowModifications.containsKey("builder-flow"),
                "builder-flow modifications should be preserved with CLEAR_NONE"
            )
        }

    @Test
    fun `should handle multiple flows with same screen type but different step parameters`() =
        runTest(timeout = 10.toDuration(DurationUnit.SECONDS)) {
            val testDispatcher = StandardTestDispatcher(testScheduler)
            ...
            assertEquals(
                "SIGNUP_PROFILE",
                state.currentEntry.params.getString("SCREEN_TYPE"),
                "Should have test-flow's screen type"
            )
            ...
            assertEquals(
                null,
                state.currentEntry.params.getBoolean("SHOW_TUTORIAL"),
                "Should NOT have route-flow's tutorial flag"
            )
            ...
            assertEquals(
                "ONBOARDING_PROFILE",
                state.currentEntry.params.getString("SCREEN_TYPE"),
                "Should have builder-flow's screen type"
            )
            ...
            assertEquals(
                true,
                state.currentEntry.params.getBoolean("SHOW_TUTORIAL"),
                "Should have builder-flow's tutorial flag"
            )
        }

    @Test
    fun `should allow modifying multiple flows simultaneously without interference`() =
        runTest(timeout = 10.toDuration(DurationUnit.SECONDS)) {
            val testDispatcher = StandardTestDispatcher(testScheduler)
            ...
            assertTrue(
                state.guidedFlowModifications.containsKey("builder-flow"),
                "Should have builder-flow modifications"
            )
            ...
            assertEquals(
                "TEST_FLOW_1",
                currentState.currentEntry.params.getString("FLOW_ID"),
                "test-flow should have its own ID"
            )
            ...
            assertEquals(
                "ROUTE_FLOW_1",
                currentState.currentEntry.params.getString("FLOW_ID"),
                "route-flow should have its own ID"
            )
            ...
            assertEquals(
                "ROUTE",
                currentState.currentEntry.params.getString("PROFILE_TYPE"),
                "route-flow should have its own profile type"
            )
            ...
            assertEquals(
                "BUILDER_FLOW_1",
                currentState.currentEntry.params.getString("FLOW_ID"),
                "builder-flow should have its own ID"
            )
            ...
            assertEquals(
                "BUILDER",
                currentState.currentEntry.params.getString("PROFILE_TYPE"),
                "builder-flow should have its own profile type"
            )
        }

    @Test
    fun `should support configurable modification clearing behavior`() =
        runTest(timeout = 15.toDuration(DurationUnit.SECONDS)) {
            val testDispatcher = StandardTestDispatcher(testScheduler)
            ...
            assertEquals(3, state.guidedFlowModifications.size, "Should have modifications for all 3 flows")
            assertTrue(
                state.guidedFlowModifications.containsKey("flow-clear-specific"),
                "Should have flow-clear-specific modifications"
            )
            assertTrue(
                state.guidedFlowModifications.containsKey("flow-clear-all"),
                "Should have flow-clear-all modifications"
            )
            assertTrue(
                state.guidedFlowModifications.containsKey("flow-clear-none"),
                "Should have flow-clear-none modifications"
            )
            ...
            assertFalse(
                state.guidedFlowModifications.containsKey("flow-clear-specific"),
                "flow-clear-specific modifications should be cleared"
            )
            assertTrue(
                state.guidedFlowModifications.containsKey("flow-clear-all"),
                "flow-clear-all modifications should remain"
            )
            assertTrue(
                state.guidedFlowModifications.containsKey("flow-clear-none"),
                "flow-clear-none modifications should remain"
            )
            ...
            assertFalse(
                state.guidedFlowModifications.containsKey("flow-clear-all"),
                "flow-clear-all modifications should be cleared"
            )
            assertFalse(
                state.guidedFlowModifications.containsKey("flow-clear-none"),
                "flow-clear-none modifications should also be cleared by CLEAR_ALL"
            )
            ...
            assertTrue(
                state.guidedFlowModifications.containsKey("flow-clear-none"),
                "flow-clear-none modifications should remain with CLEAR_NONE"
            )
        }
