...
public object LogicTracer {
    ...
    private val observers = CopyOnWriteRegistry<LogicObserver>()
     ...
     */
    public fun removeObserver(observer: LogicObserver): Boolean = observers.remove(observer)
     ...
     */
    public fun observerCount(): Int = observers.size
    ...
    public suspend fun notifyMethodStart(
        ...
        val callId = "call-${callIdCounter.addAndFetch(1L)}-$timestampMs"
        val job = coroutineContext[Job]
        val parentCallId = job?.let { CallRegistry.push(it, callId) }
    ...
    private inline fun notifyObservers(action: (LogicObserver) -> Unit) {
        observers.forEachCatching({ ReaktivDebug.warn("LogicTracer observer threw exception: ${it.message}") }, action)
    }
}
